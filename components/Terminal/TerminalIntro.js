...
const bootLines = [
  ...
  '',
  '════════════════════════════════════════════',
  'Booting WASIQ-SEC v4.2.1 (Linux Kernel 6.1.8-x64)',
  '════════════════════════════════════════════',
  '',
  '[    0.000000] Linux version 6.1.8-x64 (wasiq@secure)',
  ...
  '[    2.567890] systemd[1]: Set hostname to <wasiq-secure>',
...
const portfolioInfo = [
  ...
  '├─ Expertise: Security',
  '├─ Focus: Building secure, scalable applications',
  '╰─ Status: Open to opportunities',
  '',
  '╭─ QUICK STATS',
  '├─ Projects: 2',
  '├─ Tech Stack: C++, Python, Bash Scripting',
  '├─ Certifications: Linux Essentials, CCEP',
  '╰─ GitHub: github.com/Wasiq2006',
  '',
];
...
export default function TerminalIntro({ onComplete }) {
  ...
  return (
    <div
      ref={containerRef}
      className={`${oxanium.className} bg-black text-green-400 text-xs sm:text-sm p-4 sm:p-8 h-screen w-full flex flex-col font-mono overflow-y-auto`}
      style={{
        backgroundImage: 'radial-gradient(circle at 50% 50%, rgba(0, 255, 0, 0.03) 0%, transparent 100%)',
      }}
    >
      <AnimatePresence mode="wait">
        ...
        {phase === 'banner' && (
          ...
          >
            {/* ASCII Banner */}
            <div
              className="text-green-300 mb-6 sm:mb-8 w-full flex justify-center"
            ...
            >
              <div className="text-green-400 font-mono text-[10px] xs:text-xs sm:text-sm md:text-base break-words">
                <span className="text-cyan-300">wasiq@secure:~$ </span>
                <span>echo 'Welcome to my portfolio'</span>
              </div>

              <motion.div
                initial={{ opacity: 1 }}
                animate={{ opacity: 1 }}
                className="text-green-300 text-[10px] xs:text-xs sm:text-sm space-y-2"
              >
